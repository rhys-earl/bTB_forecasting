# Total_cases_forecasting_v2_bootstrap.py
# ------------------------------------------------------------
# Rolling seasonal forecast aligned to R (fpp3) methodology.
# Uncertainty via residual bootstrap, replicating R's generate(times=1000).
#
# CLI usage:
#   python Total_cases_forecasting_v2_bootstrap.py ARIMA
#   python Total_cases_forecasting_v2_bootstrap.py ARIMA 2016
#   python Total_cases_forecasting_v2_bootstrap.py ARIMA --transformation log1p scaler none
#   python Total_cases_forecasting_v2_bootstrap.py TiDE --gpu 1 --transformation log1p none
# ------------------------------------------------------------
...
def invert_transformation(forecast_series: TimeSeries, transformer) -> TimeSeries:
    ...
    Both the point forecast AND historical forecasts used for residuals are inverted
    before any arithmetic, so residuals, bootstrap paths, and quantiles all stay
    on the original case-count scale.
    """
...
# In-sample residuals
#
# historical_forecasts run on the TRANSFORMED series, then inverted to original
# scale before computing residuals. Actuals come from the ORIGINAL train_ts.
# This means residuals are always on the original case-count scale.
# -----------------------------
...
# Residual bootstrap
#
# point_forecast and residuals are both on the ORIGINAL scale.
# Returns (n_samples,) array of annual totals.
# -----------------------------
def residual_bootstrap_annual_samples(
    ...
    drawn_residuals = rng.choice(residuals, size=(n_samples, len(point_forecast)), replace=True)
...
def run_annual_forecast(
    ...
    for _, row in origins_df.iterrows():
        ...
        quantile_vals = {qcol(q): float(np.quantile(annual_samples, q)) for q in QUANTILES}
        ...
        logging.info(
            ...
            f"n_residuals={len(residuals)}, obs_carried={lag_cumulative:,.0f}, "
            f"mean={mean_val:,.0f}, "
            f"pi_95=({quantile_vals[qcol(0.025)]:,.0f}-{quantile_vals[qcol(0.975)]:,.0f}), "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Annual total forecast (residual bootstrap). Origins: Sep(Y-1) to Dec(Y). "
            "Replicates R's generate(times=1000) for all models."
        )
    )
    parser.add_argument(
        "model", type=str,
