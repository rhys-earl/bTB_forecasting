...
#   python Total_cases_forecasting_v2_darts.py ARIMA --test_sampling   # run correlation test only
# ------------------------------------------------------------
...
def invert_transformation(forecast_series: TimeSeries, transformer) -> TimeSeries:
    ...
    if transformer == "log1p":
        all_vals = forecast_series.all_values(copy=False)
...
# -----------------------------
# Sampling structure test
#
# Tests whether Darts generates sequential correlated paths (like R's generate())
# or independent marginal draws at each horizon step.
#
# Run with: python Total_cases_forecasting_v2_darts.py ARIMA --test_sampling
#
# What to look for:
#   Sequential simulation (same as R generate()):
#     - Adjacent step correlations clearly positive (e.g. 0.3 to 0.8)
#     - Correlation from step 1 decreases gradually with horizon distance
#     - Std dev grows with horizon (uncertainty compounds)
#
#   Independent marginal draws:
#     - All correlations near zero (-0.05 to 0.05)
#     - Std dev flat or only slightly growing across horizons
#
# If sequential -> Darts internal sampling is equivalent to R for this model.
# If independent -> bootstrap is more appropriate for annual total aggregation.
# -----------------------------
def test_sampling_structure(monthly_cases: pd.Series, model_name: str):
    print(f"\n{'='*60}")
    print(f"SAMPLING STRUCTURE TEST: {model_name}")
    print(f"{'='*60}")
    print("Fitting on first 120 months of data, forecasting 16 steps ahead.")
    print(f"Drawing {NUM_SAMPLES} samples.\n")

    train_slice = monthly_cases.iloc[:120]
    train_ts    = TimeSeries.from_series(train_slice)

    model = get_model(model_name=model_name, forecast_length=16)
    model.fit(train_ts)

    fc      = model.predict(16, num_samples=NUM_SAMPLES)
    samples = fc.all_values()[:, 0, :]  # shape (16, NUM_SAMPLES)

    # --- Correlation between adjacent steps ---
    print("Correlation between adjacent forecast steps across samples:")
    print("  (near zero = independent draws, clearly positive = sequential)")
    for i in range(15):
        corr = np.corrcoef(samples[i, :], samples[i + 1, :])[0, 1]
        bar  = "#" * int(abs(corr) * 40)
        print(f"  Step {i+1:2d} vs Step {i+2:2d}: r = {corr:+.4f}  {bar}")

    # --- Correlation from step 1 to all others ---
    print("\nCorrelation between step 1 and all subsequent steps:")
    print("  (should decrease with distance if sequential)")
    for i in range(1, 16):
        corr = np.corrcoef(samples[0, :], samples[i, :])[0, 1]
        bar  = "#" * int(abs(corr) * 40)
        print(f"  Step  1 vs Step {i+1:2d}: r = {corr:+.4f}  {bar}")

    # --- Std dev across horizons ---
    print("\nStd dev of samples at each step:")
    print("  (growing std = uncertainty compounds = sequential simulation)")
    step1_std = samples[0, :].std()
    for i in range(16):
        std   = samples[i, :].std()
        ratio = std / step1_std if step1_std > 0 else 1.0
        bar   = "#" * int(ratio * 10)
        print(f"  Step {i+1:2d}: std = {std:10,.1f}  (x{ratio:.2f} vs step 1)  {bar}")

    # --- Summary verdict ---
    mean_adj_corr = np.mean([
        np.corrcoef(samples[i, :], samples[i + 1, :])[0, 1]
        for i in range(15)
    ])
    std_ratio = samples[15, :].std() / samples[0, :].std() if samples[0, :].std() > 0 else 1.0

    print(f"\n{'='*60}")
    print("VERDICT:")
    print(f"  Mean adjacent-step correlation : {mean_adj_corr:+.4f}")
    print(f"  Std dev ratio (step 16 / step 1): {std_ratio:.2f}")
    print()

    if mean_adj_corr > 0.1 and std_ratio > 1.5:
        print("  -> SEQUENTIAL SIMULATION")
        print("     Darts internal sampling for this model is equivalent to")
        print("     R's generate(). Use Darts internal sampling for annual totals.")
    elif mean_adj_corr < 0.05 and std_ratio < 1.3:
        print("  -> INDEPENDENT MARGINAL DRAWS")
        print("     Darts draws each horizon step independently.")
        print("     The bootstrap method is more appropriate for annual totals.")
    else:
        print("  -> AMBIGUOUS")
        print("     Results are between the two extremes. Inspect the numbers above")
        print("     carefully before deciding which method to use.")
    print(f"{'='*60}\n")
...
def run_annual_forecast(
    ...
    for _, row in origins_df.iterrows():
        ...
        # Invert BEFORE extracting sample matrix
        fc = invert_transformation(fc, transformer)

        samples_matrix      = fc.all_values()[:, 0, :]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="GPU device index to use (e.g. --gpu 0). Defaults to CPU.",
    )
    parser.add_argument(
        "--test_sampling", action="store_true",
        help=(
            "Run the sampling structure test only (no forecasting). "
            "Tests whether Darts generates sequential correlated paths "
            "(equivalent to R's generate()) or independent marginal draws. "
            "Example: python script.py ARIMA --test_sampling"
        ),
    )
    ...
    if args.gpu is not None:
        if not torch.cuda.is_available():
            raise RuntimeError(f"--gpu {args.gpu} specified but no CUDA GPUs available.")
    ...
    # --test_sampling: run test and exit, no forecasting
    if args.test_sampling:
        test_sampling_structure(monthly_cases, args.model)
        exit(0)

    # Normal forecasting run
